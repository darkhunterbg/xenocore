...
namespace XenoCore.Editor
{
    ...
    public static class IconManager
    {
        ...
        public static ImageSource FindIconForDirectory(string dir, bool large)
        {
            var extension = Path.GetExtension(dir);
            if (extension == null)
                return null;
            var cache = large ? _largeIconCache : _smallIconCache;
            ImageSource icon;
            if (cache.TryGetValue(extension, out icon))
                return icon;
            icon = IconReader.GetDirectoryIcon(dir, large ? IconReader.IconSize.Large : IconReader.IconSize.Small, false).ToImageSource();
            cache.Add(extension, icon);
            return icon;
        }
        /// <summary>
        ...
        static class IconReader
        {
            ...
            public static Icon GetDirectoryIcon(string name, IconSize size, bool linkOverlay)
            {
                var shfi = new Shell32.Shfileinfo();
                var flags = Shell32.ShgfiIcon | Shell32.ShgfiUsefileattributes;
                if (linkOverlay) flags += Shell32.ShgfiLinkoverlay;
